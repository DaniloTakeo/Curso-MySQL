SELECT A.numero, COUNT(*) AS numero_itens, (B.quantidade * B.preco) AS faturado 
FROM notas_fiscais A
INNER JOIN itens_notas_fiscais B
ON A.numero = B.numero
WHERE data_venda = '2023-03-28'
GROUP BY A.numero;

CALL p_inserir_venda('2023-03-28', 10, 100);
...
BEGIN
    ...
    DECLARE vNumeroDeItensNota INT;
    ...
    DO
        ...
        SELECT COUNT(*) INTO vNumeroDeItensNota 
        FROM itens_notas_fiscais 
        WHERE numero = vNumeroNota AND codigo_do_produto = vProduto;

        IF vNumeroDeItensNota = 0 THEN
